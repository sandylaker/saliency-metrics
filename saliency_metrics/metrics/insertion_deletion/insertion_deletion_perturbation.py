...
class ProgressivePerturbation:
    def __init__(
        ...
        # TODO check num_pixels logic
        self._num_pixels = self._row_inds.size()[0]

    def _perturb_by_inds(self, row_inds: torch.Tensor, col_inds: torch.Tensor):
    ...
    def perturb(self, forward_batch_size: int = 128, perturb_step_size: int = 10):
        pixels_perturbed = 0
        # TODO - change batches to iterator and use yield instead of return
        batches = []
        while pixels_perturbed < self._num_pixels:
            ...
            while forward_batch_count < forward_batch_size:
                ...
                # TODO - check if only detach works
